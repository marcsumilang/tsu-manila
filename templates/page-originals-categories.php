...
<script type="text/x-template" id="originals-categories-page" >
    <div id='categories-content' class='inner-container'>
        ...
        <div class='categories-nav hide-on-768'>
            <router-link to='all' v-bind:class='{active: isRouteParamActive("all")}' class='category-nav'>ALL ORIGINALS</router-link>
            <?php
                for($i=0; $i<count($originals_categories_list); $i++){
                    //show category only if there are tagged videos
                    if(count($originals[$originals_categories_list[$i]->slug]) > 0){
                        echo "<router-link to='".$originals_categories_list[$i]->slug."' v-bind:class='{active: isRouteParamActive(\"".$originals_categories_list[$i]->slug."\")}' class='category-nav'>".$originals_categories_list[$i]->name."</router-link>";   
                    }
                }
            ?>
